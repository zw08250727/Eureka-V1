...
test('inline edits validate empty content and keep summary and verbatim separate', async ({ page }) => {
  ...
  await body.fill('');
  await page.getByRole('button', { name:'保存', exact:true }).click();
  await expect(page.locator('.md-inline-edit-status')).toHaveText('正文不能为空');
  await body.fill('逐字稿修订内容');
  await page.getByRole('tab', { name:'智能总结', exact:true }).click();
  await expect(page.locator('#md-content')).not.toContainText('逐字稿修订内容');
  await page.getByRole('tab', { name:'逐字稿', exact:true }).click();
  await expect(body).toHaveText('逐字稿修订内容');
  await page.getByRole('button', { name:'保存', exact:true }).click();
  await page.reload();
  await page.locator('#view-panorama').click();
  await page.getByRole('tab', { name:'逐字稿', exact:true }).click();
